...
const parseFunctionArguments = (tree) => {
  ...
  const isArrowExpression =
    maybe(_first(tree.body)).type == 'ExpressionStatement';
  const params = isArrowExpression
    ? maybe(maybe(_first(tree.body)).expression).params
    : maybe(_first(tree.body)).params;

  return _map(params, 'name');
};
...
const getCageSHA256Hash = (cageSource) => {
  return crypto.createHash('sha256').update(cageSource).digest('base64');
};
