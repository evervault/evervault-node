  ...
  return new ASN1.Sequence({
    ...
    value: [
      new ASN1.OctetString({
        name: 'a',
        valueHex: new Uint8Array(curveParams.a.match(/../g).map(h=>parseInt(h,16))).buffer
      }),
      new ASN1.OctetString({
        name: 'b',
        valueHex: new Uint8Array(curveParams.b.match(/../g).map(h=>parseInt(h,16))).buffer
      }),
      new ASN1.BitString({
        name: 'seed',
        optional: true,
        valueHex: new Uint8Array(curveParams.seed.match(/../g).map(h=>parseInt(h,16))).buffer
      }),
    ]
  });
}
...
const ECParameters = (curveParams) => {
  console.log("inside ECParameters");
  return new ASN1.Sequence({
    name: 'ECParameters',
    value: [
      new ASN1.Integer({
      ...
      new ASN1.OctetString({
        name: 'base',
        valueHex: new Uint8Array(curveParams.generator.match(/../g).map(h=>parseInt(h,16))).buffer
      }),
      new ASN1.Integer({
        name: 'order',
        value: Number(`0x${curveParams.n}`)
      }),
      new ASN1.Integer({
        name: 'cofactor',
        value: Number(`0x${curveParams.h}`)
      })
    ]
  });
}
...
const AlgorithmIdentifier = (curveParams) => {
  console.log("inside AlgorithmIdentifier");
  return new ASN1.Sequence({
    name: 'algorithm',
    value: [
      new ASN1.ObjectIdentifier({
...
const SubjectPublicKeyInfo = (curveParams, decompressedKey) => new ASN1.Sequence({
  name: 'SubjectPublicKeyInfo',
  value: [
    AlgorithmIdentifier(curveParams),
    new ASN1.BitString({
      name: 'subjectPublicKey',
      valueHex: new Uint8Array(decompressedKey.match(/../g).map(h=>parseInt(h,16))).buffer
    })
  ]
});
