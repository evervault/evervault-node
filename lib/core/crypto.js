...
module.exports = ({ config, http }) => {
  ...
  const _encryptBigBuffer = async (cageKey, data, datatype) => {
    const keyIv = await generateBytes(config.keyLength / 2);
    const rootKey = await generateBytes(config.keyLength);
    const cipher = crypto.createCipheriv(
      config.cipherAlgorithm,
    ...
    const { url } = await http.getDataUrl();
    ...
    const uploaded = await http.uploadData(url, encryptedBuffer.toString('base64'));
    if (!uploaded) throw new errors.EvervaultError('An error occurred while uploading your file to Evervault Storage.');
  ...
  const encrypt = async (key, data, options = DEFAULT_ENCRYPT_OPTIONS) => {
    ...
    if (Datatypes.isBuffer(data)) {
      return await _encryptBigBuffer(key, data, 'blob');
    } else if (Datatypes.isObjectStrict(data)) {
