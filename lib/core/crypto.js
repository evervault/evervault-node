...
module.exports = (config) => {
  ...
  const _traverseObject = async (
    ...
    if (Array.isArray(data)) {
      return _traverseArray(data, curve, ecdhTeamKey, ecdhPublicKey, derivedSecret, role);
    }
    if (Buffer.isBuffer(data)) {
      return _encryptFile(data, curve, ecdhTeamKey, ecdhPublicKey, derivedSecret, role);
    }
    if (typeof data === 'object' && data !== null) {
      return _encryptObject(data, curve, ecdhTeamKey, ecdhPublicKey, derivedSecret, role);
    }
    return _encryptString(data, curve, ecdhTeamKey, ecdhPublicKey, derivedSecret, role);
  };
   ...
   * @returns {string}
   */
  const _getDatatype = (data) => 'string';
  ...
  const _encryptString = async (
    ...
    const result = buildCipherBuffer(data, role);
