...
module.exports = (config) => {
  const _encryptObject = (cageName, cageKey, data, { fieldsToEncrypt }) => {
    ...
    for (let key of keys) {
      encryptedObject[key] = _encryptString(cageName, cageKey, data[key]);
    }
  ...
  const encrypt = (cageName, key, data, options = {}) => {
    ...
    if (
      ...
      return _encryptObject(cageName, cryptoKey, data, options);
    } else {
      return _encryptString(cageName, cryptoKey, Datatypes.ensureString(data));
    }
  };
