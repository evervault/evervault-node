...
module.exports = (config) => {
  ...
  const _encryptData = (data, setAuthData, derivedSecret, ecdhTeamKey, keyIv) => {
    const cipher = crypto.createCipheriv(
      config.cipherAlgorithm,
      derivedSecret,
      keyIv,
  ...
  const _encryptFile = async (
    ...
    // if (curve === PRIME256V1 || role) {
    //   cipher.setAAD(Buffer.from(ecdhTeamKey, 'base64'));
    // }

    // const setAuthData = (curve === PRIME256V1 || role) ? true : false;
    const setAuthData = true;
    console.log(setAuthData);
    const encryptedBuffer = _encryptData(data, setAuthData, derivedSecret, ecdhTeamKey, keyIv);
    ...
    // if (role) {
      ...
      encryptedMetadataBytes = _encryptData(metadataBytes, setAuthData, derivedSecret, ecdhTeamKey, keyIv);
    //}
    ...
    return _formatFile(keyIv, ecdhPublicKey, encryptedBuffer, encryptedMetadataBytes);
  };
  ...
  const _formatFile = async (keyIv, ecdhPublicKey, encryptedData, encryptedMetadataBytes = undefined) => {
    ...
    console.log("KEY", Buffer.from(keyIv))
    let fileContents;
    if (encryptedMetadataBytes) {
      let metadataOffsetBuffer = Buffer.allocUnsafe(2);
      metadataOffsetBuffer.writeUInt16LE(encryptedMetadataBytes.length);
      console.log("META", metadataOffsetBuffer);
      console.log("ACT", encryptedMetadataBytes)
      console.log("DAT", encryptedData)
