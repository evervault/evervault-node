...
class EvervaultClient {
  ...
  _overloadHttpsModule(apiKey, tunnelHostname) {
    ...
    const cert_file = createWriteStream(join(__dirname, 'rootCA.crt'));
    const cert_host = 'https://ca.evervault.com';
    https.get(cert_host, (res) => {
      res.pipe(cert_file);
      cert_file.on('finish', () => {
        cert_file.close();
      })
      .on('error', (err) => {
        throw new errors.CertError(`Unable to download cert from ${cert_host} (${err.message})`);
      });
    });
    ...
    tls.createSecureContext = (options) => {
      ...
      const pem = readFileSync(join(__dirname, 'rootCA.crt'), { encoding: "ascii" })
