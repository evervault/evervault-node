...
class EvervaultClient {
  ...
  constructor(apiKey, options = {}) {
    ...
    if (options.intercept || options.ignoreDomains) {
      console.warn(
        '\x1b[43m\x1b[30mThe `intercept` and `ignoreDomains` config options in Evervault Node.js SDK are deprecated and slated for removal.',
   ...
   * @returns {void}
   */
  async _overloadHttpsModule(apiKey, tunnelHostname, domainSelector) {
    let x509 = null;
    let evClient = this;
    ...
    function wrapMethodRequest(...args) {
      const shouldProxy = domainSelector(args);
      args = args.map((arg) => {
  ...
  _isDecryptionDomain(args, decryptionDomains) {
    for (const arg of args) {
      if (arg instanceof Object) {
        return (
          decryptionDomains.includes(arg.host) ||
          decryptionDomains.includes(arg.hostname)
        );
