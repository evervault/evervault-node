...
class EvervaultClient {
  ...
  /** @private @type {import('./config')} */ config;
  /** @private @type {import('./config').SupportedCurve} */ curve;
  ...
  /**
   * @param {Record<string, import('@evervault/sdk').AttestationData | import('@evervault/sdk').AttestationCallback>} attestationData
   * @param {import('@evervault/sdk').AttestationBindings} attestationBindings
  ...
  _alwaysIgnoreDomains() {
    ...
    const enclavesHost = new URL(this.config.http.enclavesHostname).host;

    return [caHost, apiHost, enclavesHost];
  }
