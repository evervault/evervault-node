...
class EvervaultClient {
  ...
  constructor(apiKey, options = {}) {
    ...
    if (options.intercept || options.ignoreDomains) {
      console.warn('\x1b[43m\x1b[30mThe `intercept` and `ignoreDomains` config options in Evervault Node.js SDK are deprecated and slated for removal.',
        '\nPlease switch to the equivalent `decryptionDomains` config option.',
        '\nMore details: https://docs.evervault.com/reference/nodejs-sdk#evervaultsdk\x1b[0m');
    }

    if (options.decryptionDomains && options.decryptionDomains.length > 0) {
      const decryptionDomainsFilter = this._decryptionDomainsFilter(options.decryptionDomains);
      this._overloadHttpsModule(
        apiKey,
        this.config.http.tunnelHostname,
        decryptionDomainsFilter
      );
    } else if (options.intercept === true || options.relay === true) {
      const ignoreDomainsFilter = this._ignoreDomainFilter(options.ignoreDomains);
      this._overloadHttpsModule(
        apiKey,
        this.config.http.tunnelHostname,
        ignoreDomainsFilter
   ...
   * @returns {void}
   */
  async _overloadHttpsModule(apiKey, tunnelHostname, domainFilter) {
    let x509 = null;
    let evClient = this;
    ...
    function wrapMethodRequest(...args) {
      const ignore = domainFilter(args);
      args = args.map((arg) => {
  ...
  _decryptionDomainsFilter(decryptionDomains) {
    return (args) => !this._isDecryptionDomain(args, decryptionDomains);
  }

  _isDecryptionDomain(args, decryptionDomains) {
    for (const arg of args) {
      if (arg instanceof Object) {
        return decryptionDomains.includes(arg.host) || decryptionDomains.includes(arg.hostname);
      } else if (typeof arg === 'string') {
        return decryptionDomains.includes(arg);
      }
    }
