...
class EvervaultClient {
  ...
  async _overloadHttpsModule(apiKey, tunnelHostname, ignoreDomains) {
    ...
    if (!ignoreDomains) ignoreDomains = []
    ...
    const _exactOrEndsWith = (domain, exactDomains, endsWithDomains) => {
      if (exactDomains.includes(domain)) return true;
      for (let end of endsWithDomains) {
        if (domain?.endsWith(end)) return true;
      }
      return false;
    };
    ...
    https.request = function wrapMethodRequest(...args) {
      ...
      for (let arg of args) {
        if (arg instanceof Object) {
          ignore = _exactOrEndsWith(arg.hostname, ignoreExact, ignoreEndsWith)
            || _exactOrEndsWith(arg.host, ignoreExact, ignoreEndsWith);
        }
        else if (arg instanceof string) ignore = _exactOrEndsWith(arg, ignoreExact, ignoreEndsWith);
