...
class EvervaultClient {
  ...
  async _overloadHttpsModule(apiKey, tunnelHostname, ignoreDomains = []) {
    ...
    async function update_certificate() {
    ...
    function is_certificate_unavaliable() {
      return x509 == null;
    }

    function is_certificate_expired() {
      if (x509 == null) {
        return false;
      }
      let ts = Date.now();
      ...
      return (
        date.getUTCDate() > x509.validTo || date.getUTCDate() < x509.validFrom
      );
    }
