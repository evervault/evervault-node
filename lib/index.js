...
class EvervaultClient {
   ...
   * @returns {Function}
   */
  cagify(func, cageName = undefined) {
    if (!Datatypes.isFunction(func)) {
      throw new errors.EvervaultError('Cagify must be provided with a function to run');
    }

    const { cageName: autoCageName, cageHash, functionRequires, functionParameters } = sourceParser.parseSource(func);
    cageName = cageName || autoCageName;
    ...
    if (cageLock.deployCheck(cageName, cageHash)) {
        ...
        cageLock.addCageToLockfile(cageName, cageHash, deployedBy, deployedTeam);
    }
