...
class EvervaultClient {
  ...
  constructor(apiKey, appId, options = {}) {
    if (appId === '' || !Datatypes.isString(appId)) {
      throw new errors.InitializationError(
        'The app ID must be a string and cannot be empty.'
      );
    }
   ...
   */
  async decrypt(data) {
    if (!Datatypes.isObject(data) && !Datatypes.isBuffer(data)) {
      throw new errors.EvervaultError(
        '`decrypt()` must be called with an object or buffer.'
      );
    }
    const response = await this.http.decrypt(data);
